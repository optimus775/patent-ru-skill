...
- `prior_art_notes.md`, `patentability_overlap_report_*.md` и `preview.md` - проверяемые рабочие материалы;
...
- рабочий отчет Step 5.5 содержит предварительную оценку перекрытия, обязательное ограничение и статус по шкале из трех уровней;
